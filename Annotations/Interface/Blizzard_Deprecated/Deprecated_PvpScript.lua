---@meta _
---@deprecated
---Deprecated by [C_PvP.IsSubZonePVPPOI](https://warcraft.wiki.gg/wiki/API_C_PvP.IsSubZonePVPPOI)
---@return boolean result
function IsSubZonePVPPOI() end

---@deprecated
---Deprecated by [C_PvP.GetZonePVPInfo](https://warcraft.wiki.gg/wiki/API_C_PvP.GetZonePVPInfo)
---@return string pvpType
---@return boolean isSubZonePvP
---@return string? factionName
function GetZonePVPInfo() end

---@deprecated
---Deprecated by [C_PvP.TogglePVP](https://warcraft.wiki.gg/wiki/API_C_PvP.TogglePVP)
function TogglePVP() end

---@deprecated
---Deprecated by [C_PvP.SetPVP](https://warcraft.wiki.gg/wiki/API_C_PvP.SetPVP)
---@param enablePVP? boolean Default = false
function SetPVP(enablePVP) end

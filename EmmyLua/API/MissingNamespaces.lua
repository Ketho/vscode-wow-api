---@meta
-- these namespaces didnt document any functions
C_AdventureJournal = {} -- AdventureJournalDocumentation.lua
C_AdventureMap = {} -- AdventureMapDocumentation.lua
C_BlackMarket = {} -- BlackMarketInfoDocumentation.lua
C_CharacterServices = {} -- no doc
C_CharacterServicesPublic = {} -- no doc
C_ClassTrial = {} -- ClassTrialDocumentation.lua
C_Debug = {} -- no doc
C_Heirloom = {} -- HeirloomInfoDocumentation.lua
C_LootHistory = {} -- no doc
C_NamePlate = {} -- NamePlateManagerDocumentation.lua
C_NewItems = {} -- no doc
C_PrototypeDialog = {} -- no doc
C_Scenario = {} -- ScenarioInfoDocumentation.lua
C_SharedCharacterServices = {} -- no doc
C_TalkingHead = {} -- TalkingHeadDocumentation.lua
C_Timer = {} -- no doc
C_ToyBox = {} -- no doc
C_Trophy = {} -- no doc
C_Widget = {} -- no doc
C_WowTokenPublic = {} -- no doc

C_LootJournal = {} -- LootJournalDocumentation.lua not exported
C_FunctionContainers = {}

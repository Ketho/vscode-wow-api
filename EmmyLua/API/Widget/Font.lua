---@meta
---[Documentation](https://wowpedia.fandom.com/wiki/UIOBJECT_Font)
---@class Font
local Font = {}

---[Documentation](https://wowpedia.fandom.com/wiki/API_Font_CopyFontObject)
---@param sourceFont SimpleFont
function Font:CopyFontObject(sourceFont) end

---[Documentation](https://wowpedia.fandom.com/wiki/API_Font_GetAlpha)
---@return SingleColorValue alpha
function Font:GetAlpha() end

---[Documentation](https://wowpedia.fandom.com/wiki/API_Font_GetFont)
---@return string fontFile
---@return uiFontHeight height
---@return TBFFlags flags
function Font:GetFont() end

---[Documentation](https://wowpedia.fandom.com/wiki/API_Font_GetFontObject)
---@return SimpleFont font
function Font:GetFontObject() end

---[Documentation](https://wowpedia.fandom.com/wiki/API_Font_GetIndentedWordWrap)
---@return boolean wordWrap
function Font:GetIndentedWordWrap() end

---[Documentation](https://wowpedia.fandom.com/wiki/API_Font_GetJustifyH)
---@return JustifyH justifyH
function Font:GetJustifyH() end

---[Documentation](https://wowpedia.fandom.com/wiki/API_Font_GetJustifyV)
---@return JustifyV justifyV
function Font:GetJustifyV() end

---[Documentation](https://wowpedia.fandom.com/wiki/API_Font_GetShadowColor)
---@return number colorR
---@return number colorG
---@return number colorB
---@return number colorA
function Font:GetShadowColor() end

---[Documentation](https://wowpedia.fandom.com/wiki/API_Font_GetShadowOffset)
---@return number offsetX
---@return number offsetY
function Font:GetShadowOffset() end

---[Documentation](https://wowpedia.fandom.com/wiki/API_Font_GetSpacing)
---@return uiUnit spacing
function Font:GetSpacing() end

---[Documentation](https://wowpedia.fandom.com/wiki/API_Font_GetTextColor)
---@return number colorR
---@return number colorG
---@return number colorB
---@return number colorA
function Font:GetTextColor() end

---[Documentation](https://wowpedia.fandom.com/wiki/API_Font_SetAlpha)
---@param alpha SingleColorValue
function Font:SetAlpha(alpha) end

---[Documentation](https://wowpedia.fandom.com/wiki/API_Font_SetFont)
---@param fontFile string
---@param height uiFontHeight
---@param flags TBFFlags
function Font:SetFont(fontFile, height, flags) end

---[Documentation](https://wowpedia.fandom.com/wiki/API_Font_SetFontObject)
---@param font SimpleFont
function Font:SetFontObject(font) end

---[Documentation](https://wowpedia.fandom.com/wiki/API_Font_SetIndentedWordWrap)
---@param wordWrap boolean
function Font:SetIndentedWordWrap(wordWrap) end

---[Documentation](https://wowpedia.fandom.com/wiki/API_Font_SetJustifyH)
---@param justifyH JustifyH
function Font:SetJustifyH(justifyH) end

---[Documentation](https://wowpedia.fandom.com/wiki/API_Font_SetJustifyV)
---@param justifyV JustifyV
function Font:SetJustifyV(justifyV) end

---[Documentation](https://wowpedia.fandom.com/wiki/API_Font_SetShadowColor)
---@param colorR number
---@param colorG number
---@param colorB number
---@param a? SingleColorValue
function Font:SetShadowColor(colorR, colorG, colorB, a) end

---[Documentation](https://wowpedia.fandom.com/wiki/API_Font_SetShadowOffset)
---@param offsetX number
---@param offsetY number
function Font:SetShadowOffset(offsetX, offsetY) end

---[Documentation](https://wowpedia.fandom.com/wiki/API_Font_SetSpacing)
---@param spacing uiUnit
function Font:SetSpacing(spacing) end

---[Documentation](https://wowpedia.fandom.com/wiki/API_Font_SetTextColor)
---@param colorR number
---@param colorG number
---@param colorB number
---@param a? SingleColorValue
function Font:SetTextColor(colorR, colorG, colorB, a) end

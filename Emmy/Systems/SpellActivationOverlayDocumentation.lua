---@class SPELL_ACTIVATION_OVERLAY_GLOW_HIDE
---@field spellID number
---[Documentation](https://wow.gamepedia.com/SPELL_ACTIVATION_OVERLAY_GLOW_HIDE)
local SPELL_ACTIVATION_OVERLAY_GLOW_HIDE = {}

---@class SPELL_ACTIVATION_OVERLAY_GLOW_SHOW
---@field spellID number
---[Documentation](https://wow.gamepedia.com/SPELL_ACTIVATION_OVERLAY_GLOW_SHOW)
local SPELL_ACTIVATION_OVERLAY_GLOW_SHOW = {}

---@class SPELL_ACTIVATION_OVERLAY_HIDE
---@field spellID number
---[Documentation](https://wow.gamepedia.com/SPELL_ACTIVATION_OVERLAY_HIDE)
local SPELL_ACTIVATION_OVERLAY_HIDE = {}

---@class SPELL_ACTIVATION_OVERLAY_SHOW
---@field spellID number
---@field overlayFileDataID number
---@field locationName string
---@field scale number
---@field r number
---@field g number
---@field b number
---[Documentation](https://wow.gamepedia.com/SPELL_ACTIVATION_OVERLAY_SHOW)
local SPELL_ACTIVATION_OVERLAY_SHOW = {}
